...
public class ReaderAdapter extends ArrayAdapter<DocGia> implements Filterable {

    private SparseBooleanArray selectedListItemsIds;
    private List multipleSelectionList;
    private CommonVariables commonVariables;
    private List<DocGia> docGiaList, mStringFilterList;
    private Context mContext;
    NguoiDungFilter docGiaFilter;

    public ReaderAdapter(@NonNull Context context, @NonNull List<DocGia> objects) {
        super(context, 0, objects);
        selectedListItemsIds = new SparseBooleanArray();
        this.multipleSelectionList = objects;
        this.docGiaList = objects;
    ...
    @Override
    public DocGia getItem(int i) {
        return docGiaList.get(i);
    }
    ...
    private class ViewHolder {
        ...
        ImageView imgAnhDocGia, imgMenuDocGia;
    }
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if (listView == null) {
            ...
            holder.imgAnhDocGia = listView.findViewById(R.id.imgAnhUser);
            ...
            holder.imgMenuDocGia = listView.findViewById(R.id.menuUser);
            listView.setTag(holder);
        } else {
            holder = (ViewHolder) listView.getTag();
        }

        final DocGia nguoiDungAdapter = getItem(position);
        ...
        Picasso.get().load(APIUtils.Base_Url + "images/" + (nguoiDungAdapter.getAnhDocGia()))
                .placeholder(R.mipmap.ic_avatar)
                .error(R.mipmap.ic_book_error)
                .fit()
                .into(holder.imgAnhDocGia);

        holder.imgMenuDocGia.setOnClickListener(new View.OnClickListener() {
            @Override
...
//                        }
    ...
    @Override
    public void remove(DocGia docGia) {
        multipleSelectionList.remove(docGia);
    ...
    public void xepKetQua(List<DocGia> filterList) {
        commonVariables = (CommonVariables) getContext().getApplicationContext();
        final String xep = commonVariables.getTimTheo();
        Collections.sort(filterList, new Comparator<DocGia>() {
            @Override
            public int compare(DocGia d1, DocGia d2) {
                if (!xep.isEmpty()) {
                    switch (xep) {
...
//                            }
    ...
    public Filter getFilter() {
        if (docGiaFilter == null) {
            docGiaFilter = new NguoiDungFilter();
        }
        return docGiaFilter;
    }
    ...
    private class NguoiDungFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint != null && constraint.length() > 0) {
                ArrayList<DocGia> filterList = new ArrayList<DocGia>();
                for (int i = 0; i < docGiaList.size(); i++) {
                    if (removeAccent(docGiaList.get(i).getTen().toUpperCase()).contains(removeAccent(constraint.toString().toUpperCase()))) {
                        DocGia docGia = docGiaList.get(i);
                        filterList.add(docGia);
                    }
                }
...
//            if (!bienToanCuc.getTimTheo().equals("Không") && bienToanCuc!=null) {
            docGiaList = (ArrayList<DocGia>) results.values;
//            xepKetQua(docGiaList);
//            }
            notifyDataSetChanged();
        }
    }
